...
# cuSparse
_libcusparse = ctypes.cdll.LoadLibrary('cusparse64_80.dll')
...
# cuSOLVER
_libcusolver = ctypes.cdll.LoadLibrary('cusolver64_80.dll')
...
_libcusolver.cusolverSpDcsreigvsi.argtypes= [ctypes.c_void_p, #cuso_handle
                                            ctypes.c_int, #m
                                            ctypes.c_int, #nnz
...
def csreigvsi(Acsr, mu0, neigs=1, tol=1e-10, maxite=1000):
    ...
    mu0 = ctypes.c_double(mu0) # Convert mu0 to c_type
    
    mu = np.float64(mu0) # Create float64 to store mu, seed with mu0
    ...
    x0 = np.ones(Acsr.shape[0]) # Seed x0 with ones
...
if __name__ == "__main__":
    ...
    mu0 = 425.1 # Initial eigenvalue guess
